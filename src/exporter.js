...
const projectReportDefinition = ['STT','Mã dự án','Mã hàng','Tên hàng','Số lượng BOOM','Số liệu XK','Maker','Ngày bắn code','Ngày nhập kho','Số lượng nhập kho','Note','Note đổi mã','Đổi PR'];
...
function formatProjectReportSheet(ws, rows) {
  ...
  rows.forEach((row, index) => {
    const output = ws.addRow([
      ...
      row.warehouseDate, row.warehouseQuantity, exportNote(row), '', ''
    ]);
    ...
    if (row.originalItemCode && row.replacementItemCode) {
      ...
      output.getCell(12).value = replacementText(row.originalItemCode, row.replacementItemCode);
    }
    if (row.originalPurchaseOrder && row.replacementPurchaseOrder) output.getCell(13).value = replacementText(row.originalPurchaseOrder, row.replacementPurchaseOrder);
    [1,2,5,6,7,8,9,10].forEach(col => output.getCell(col).alignment = { horizontal:'center', vertical:'middle' });
    [11,12,13].forEach(col => { output.getCell(col).alignment = { vertical:'top', wrapText:true }; });
  ...
  const widths = [9,17,29,29,14,14,14,14,14,17,28,35,35];
  widths.forEach((width, index) => { ws.getColumn(index + 1).width = width; });
  [5,6,10].forEach(col => { ws.getColumn(col).numFmt = '#,##0.######'; });
  ws.views = [{ state:'frozen', xSplit:5, ySplit:9, topLeftCell:'F10', activeCell:'A10' }];
  ws.autoFilter = { from:'A9', to:`M${Math.max(9, 9 + rows.length)}` };
...
function exportNote(row) {
  ...
  const received = numeric(row.warehouseQuantity);
  if (scanned > purchase + 1e-8) return 'Bắn code thừa';
  if (scanned < purchase - 1e-8) return received < purchase - 1e-8 ? 'Hàng chưa về' : 'Chưa quét mã';
  return '';
}
