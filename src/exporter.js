...
const projectReportDefinition = ['STT','Mã dự án','Mã hàng','Tên hàng','Số lượng BOOM','Số liệu XK','Maker','Ngày bắn code','Ngày nhập kho','Số lượng nhập kho','Note'];
...
function formatProjectReportSheet(ws, rows) {
  ...
  rows.forEach((row, index) => {
    const output = ws.addRow([
      ...
      row.warehouseDate, row.warehouseQuantity, row.note
    ]);
    ...
    if (row.originalItemCode && row.replacementItemCode) {
      output.getCell(3).value = { richText: [
        { font:{ name:'Aptos Narrow', size:11, strike:true, color:{ argb:'FF7A8791' } }, text:String(row.originalItemCode) },
        { font:{ name:'Aptos Narrow', size:11, bold:true }, text:` → ${row.replacementItemCode}` }
      ] };
    }
    [1,2,5,6,7,8,9,10].forEach(col => output.getCell(col).alignment = { horizontal:'center', vertical:'middle' });
    output.getCell(11).alignment = { vertical:'top', wrapText:true };
  ...
  const widths = [9,17,29,29,14,14,14,14,14,17,42];
  ...
  ws.autoFilter = { from:'A9', to:`K${Math.max(9, 9 + rows.length)}` };
