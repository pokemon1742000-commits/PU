...
function buildComparison(purchases, scans, warehouse, threshold = 91, decisions = new Map(), confirmationThreshold = 90, purchaseReplacements = []) {
  ...
  for (const scan of comparisonScans) {
    ...
    comparison.push({
      ...
      purchaseMatchedCode:purchaseMatch.code || '', warehouseMatchedCode:warehouseMatch.code || '',
      purchaseMatchKind:purchaseMatch.kind, warehouseMatchKind:warehouseMatch.kind,
      purchaseMatchScore:purchaseMatch.score, warehouseMatchScore:warehouseMatch.score,
      receiptSource:[...new Set(matchedWarehouse.map(row => row.sourceKind === 'workshop' ? 'Xưởng Gia Công' : 'Nhập Kho'))].join('; '),
  ...
  for (const key of remainingKeys) {
    ...
    comparison.push({
      ...
      purchaseMatchedCode:purchaseRows[0]?.itemCode || '', warehouseMatchedCode:warehouseRows[0]?.itemCode || '',
      purchaseMatchKind:purchaseRows.length ? 'exact' : 'different', warehouseMatchKind:warehouseRows.length ? 'exact' : 'different',
      purchaseMatchScore:purchaseRows.length ? 100 : 0, warehouseMatchScore:warehouseRows.length ? 100 : 0,
      receiptSource:[...new Set(warehouseRows.map(row => row.sourceKind === 'workshop' ? 'Xưởng Gia Công' : 'Nhập Kho'))].join('; '),
